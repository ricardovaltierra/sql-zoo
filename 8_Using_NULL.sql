...
SELECT name, CASE WHEN mobile IS NULL THEN '07986 444 2266' ELSE mobile END FROM teacher;

-- 6. Print the teacher name and department name. Use the string 'None' where there is no department.
SELECT teacher.name, CASE WHEN dept.name IS NULL THEN 'None' ELSE dept.name END FROM teacher LEFT JOIN dept ON teacher.dept = dept.id;
...
-- the teacher is in dept 1 or 2, show 'Art' if the teacher's dept is 3 and 'None' otherwise. 

...
SELECT name, CONCAT(ROUND(ROUND(population/(SELECT population FROM world WHERE name = 'Germany'), 2)*100, 0), '%') FROM world WHERE continent = 'Europe';
...
SELECT continent, name FROM world x WHERE name <= ALL (SELECT name FROM world y WHERE y.continent=x.continent AND area>0);

-- 9. Find the continents where all countries have a population <= 25000000. 
-- Then find the names of the countries associated with these continents. Show name, continent and population. 
SELECT name, continent, population FROM world WHERE continent in (SELECT DISTINCT continent FROM world x WHERE 25000000 >= (SELECT MAX(population) FROM world y WHERE x.continent = y.continent));

...
SELECT company, num, COUNT(*) FROM route WHERE stop=53 OR stop=149 GROUP BY company, num HAVING COUNT(*) = 2;

-- 5. Change the query so that it shows the services from Craiglockhart to London Road. 
SELECT x.company, x.num, x.stop, y.stop FROM route x JOIN route y ON x.company=y.company AND x.num=y.num WHERE x.stop=53 AND y.stop = 149

-- 6. Change the query so that the services between 'Craiglockhart' and 'London Road' are shown. 
-- If you are tired of these places try 'Fairmilehead' against 'Tollcross' 
SELECT x.company, x.num, stopx.name, stopy.name FROM route x JOIN route y ON (x.company=y.company AND x.num=y.num)
	JOIN stops stopx ON (x.stop=stopx.id) 
	JOIN stops stopy ON (y.stop=stopy.id) WHERE stopx.name='Craiglockhart' AND stopy.name = 'London Road';
...
SELECT  a.num, a.company, stopb.name, d.num, d.company FROM route a 
	JOIN route b ON a.company=b.company AND a.num=b.num 
	JOIN route c 
